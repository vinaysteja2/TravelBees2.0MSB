...
@GetMapping("/touristGuideId/{id}")
public ResponseEntity<?> getTouristGuideById1(@PathVariable Long id) {
	logger.info("Http request to get touristGuideDetails");
    TouristGuideDto touristGuideDto = touristGuideServiceImpl.getTouristGuideById(id);
    if (touristGuideDto != null) {
    	 return ResponseEntity.status(HttpStatus.CREATED).body(touristGuideDto);
    ...
    @PutMapping("/{touristGuideId}")
    public ResponseEntity<Void> updateTouristGuide(@PathVariable Long touristGuideId,  @RequestBody TouristGuide updatedTouristGuide) {
        try {
